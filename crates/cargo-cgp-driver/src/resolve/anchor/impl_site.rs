...
use crate::resolve::cgp_item::{
    consumer_provider_trait, is_blanket_trait, is_local_adt, is_provider_trait,
};
...
fn wrapper_consumer_causes<'tcx>(
        ...
        .skip_binder()
    {
        ...
        // Only a CGP trait supertrait is the wiring failure the wrapper surfaces; a plain
        ...
        if consumer_provider_trait(tcx, sup_did).is_none() && !is_blanket_trait(tcx, sup_did) {
            continue;
        }
