...
The comparison with Clippy is a standing part of every document, not an aside. Clippy is the
reference implementation of this exact integration, so each document states where `cargo-cgp`
follows Clippy and where it deliberately diverges, and names the reason for each divergence. When a
divergence is a simplification `cargo-cgp` has not yet needed to undo — an argument form it does not
handle, a compiler hook it does not install — record it as a gap rather than implying parity, so a
later agent inherits the map of what is missing instead of rediscovering it.
...
source — and, for a claim about compiler or Clippy behavior, against the read-only sources under
[`../../../external`](../../../external) — before writing it.
...
the compiler-API access). Every document then closes with three standing sections:

- **Comparison with Clippy** — where the design follows `clippy-driver`/`cargo-clippy` and where it
  diverges, with the reason for each divergence, and the gaps where `cargo-cgp` is deliberately
  simpler than Clippy today.
...
for it — most naturally in the Comparison with Clippy section, since today most gaps are behaviors
Clippy handles and `cargo-cgp` does not yet. Describe the behavior as it currently is, say what the
