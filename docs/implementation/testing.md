...
`wiring/` for the remaining classes (`wiring/orphan/` holding the cross-crate orphan-rule cases the
tool now reshapes into `[CGP-E011]`). `usability/` is split by the kind of issue that remains —
`lowering/` and `wiring/` (currently just `constraints/`).
