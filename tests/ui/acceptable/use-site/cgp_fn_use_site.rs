//! A `#[cgp_fn]` trait called at a **use site**, on a context that cannot satisfy it.
//!
...
//! `DelegateComponent`). Such a trait is most naturally exercised by simply calling its method
//! on a value: `app.describe()`. When the context is missing a field one of the composed
//! traits reads, CGP's lazy wiring lets the call type-check far enough that the failure
...
//! (`acceptable/fields/cgp_fn_missing_field.rs`), which asserts the same trait through a
//! wrapper `impl` block. Here there is no wrapper and no check — the trait is called directly,
...
//! `App: Describe` to the missing field. Because a `#[cgp_fn]` trait is not a CGP *component*,
...
/// A second `#[cgp_fn]` trait that reads a `count` field from its context.
#[cgp_fn]
pub fn get_count(&self, #[implicit] count: &u64) -> u64 {
    *count
}

/// A composite `#[cgp_fn]` trait that uses both, so its blanket impl depends on
