//! A `#[cgp_fn]` trait required through a `where` **bound**, not called as a method.
//!
...
//! directly, such a trait is commonly required as a bound on a generic function
//! (`fn greet_all<Context: GetName>(…)`), so the function works on any context providing it. When
//! that function is called with a context missing the field the trait reads, the failure is an
//! `E0277` on the *call*, and the diagnostic names the blanket trait `GetName` (in the bound and
...
//! blanket trait the diagnostic names in its spans.
//!
//! The by-blanket-trait use-site anchor recovers it: it finds the local `#[cgp_fn]`/`#[blanket_trait]`
...
//! (trait-used-as-a-bound) shape and tried after the call-site anchor, so a direct method call
//! still leads with the trait the programmer invoked. Left to raw rustc the field name is
...
/// A generic function that requires the trait as a `where` bound rather than calling it on a
/// concrete context.
