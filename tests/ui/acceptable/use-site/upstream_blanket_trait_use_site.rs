//! A `#[cgp_fn]` trait defined **upstream**, called on a context that cannot satisfy it.
//!
//! This is `acceptable/use-site/cgp_fn_use_site.rs` with one thing changed: the trait
//! lives in another crate. That is the normal arrangement — a library publishes traits
//! and an application consumes them — and it is the arrangement the recognition used to miss.
//!
//! A `#[cgp_fn]`/`#[blanket_trait]` blanket trait is not a CGP component, so no marker or
...
//! every published trait along with the std blankets it was aimed at, and the
//! `[CGP-E009]` reshaping stopped the moment a trait moved into a library.
//!
//! A foreign trait now qualifies on positive evidence instead: its blanket must depend on a CGP
//! construct, followed through composed traits. Here `Describe` depends on `HasName`,
...
#[derive(HasField)]
pub struct App {
    // No `name` field, so the upstream `HasName` the trait composes cannot hold.
    pub count: u64,
}
